...
class RasterPlotWidget(AbstractWidget):
  ...
  def render_ticks(self, time_step: int) -> None:
    ...
    gl.glEnable(gl.GL_SCISSOR_TEST)     # Fixed area to be drawn in
    gl.glScissor(
      self.x_tick_x,
      self.x_tick_y,
      self.x_tick_width,
    ...
    gl.glClear(gl.GL_COLOR_BUFFER_BIT)  # Set color to white

    ### Calculate tick labels and position vertices ###
    # TODO: Potentially move this to GPU (Honestly, probably not necessary)
